...
This program allows for an arbitray image to be passed in to it. It then tests
several implementations of different algorithms, including Lempel-Ziv, to 
compress and decompress these files.
----------------------------------------------------------
These are references I used to implement the Lempel-Ziv algortihm in C++
References: 
...
void printInstructions() {
    std::cout << std::endl << "*******Instructions*******" << std::endl <<
        ...
        "    'AlgX' is the algorithm to be used, currently 'LZ'" << std::endl <<
    ...
    if (globals::allowedFileTypes.find(savedExtension) != globals::allowedFileTypes.end()) { //If file type is allowed      
        ...
        if(std::string("-c") == argv[1]){ 
            ...
            switch ( switchHash(algorithmChoice) ){
                // case switchHash("pngShrink"): {
                //     std::ofstream outputFile(exactFileName + "_PNGcompressed." + savedExtension, std::ios_base::binary);        
                //     pngShrink(inputFile, outputFile);
                //     break;
                // }
                case switchHash("LZ"): {
        ...
        /*Decompression Algorithms*/
            ...
            switch ( switchHash(algorithmChoice) ){
                // case switchHash(""): {
                //     std::ofstream outputFile(exactFileName + "_decompressed." + savedExtension, std::ios_base::binary);        
                //     bmpDecompress(inputFile, outputFile);
                //     break;
                // }
                case switchHash("LZ"):{
